...
schema:
	curl -X POST \
		...
		--data-binary "@./docker/data/improved_schema.json" \
...
populate:
	curl -X POST \
		-H 'Content-type:application/json' \
		--data-binary "@./docker/data/dataset_final_cleaned.json" \
		http://localhost:8983/solr/songs/update?commit=true
...
populate:
	curl -X POST \
		-H 'Content-type:application/json' \
		--data-binary "@./docker/data/semantic_songs.json" \
		http://localhost:8983/solr/songs/update?commit=true
